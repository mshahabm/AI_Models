...
- Brand is standardized as a single `brand` column.
  - Legacy month-specific columns like `brand_MM_YYYY` are consolidated and removed.
...
- Columns: account_number, account_id, Age, brand, health_plan, member name, care manager, Data_Quality_Pct, Risk_Score (1-10), Risk_Category (Low/Moderate/High)
- One row per patient
...
- Incrementally updates member roster (new members added, canceled members removed)
