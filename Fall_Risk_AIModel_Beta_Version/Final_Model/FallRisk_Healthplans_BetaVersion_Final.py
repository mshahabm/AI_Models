...
class FallRiskForecastingModel:
    ...
    def _create_model(self, class_weights=None):
        ...
        if self.algorithm == 'XGBoost':
            scale_pos_weight = 47.5  # Option 3.5 (Intermediate): balanced between 40.0 (Option 3++) and 55.0 (Option 4)
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, sample_weights=None):
        ...
        # Option 3.5 (Intermediate): balanced between 0.003 (Option 3++) and 0.0015 (Option 4)
        if self.optimal_threshold > 0.002:
            original_threshold = self.optimal_threshold
            self.optimal_threshold = 0.002
            print(f"    ⚠ Threshold lowered from {original_threshold:.4f} to {self.optimal_threshold:.3f} for improved recall (option 3.5)")
