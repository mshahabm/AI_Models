...
class FallRiskForecastingModel:
    ...
    def __init__(self, algorithm='XGBoost', use_temporal_features=True, use_smote=None):
        ...
        self.optimal_threshold = 0.0055
        
        # Adaptive thresholds: lower for early months (less training data), higher for later months
        self.adaptive_thresholds = {
            'May_2025': 0.0025, 'Jun_2025': 0.0030, 'Jul_2025': 0.0035,
            'Aug_2025': 0.0040, 'Sep_2025': 0.0045, 'Oct_2025': 0.0050,
            'Nov_2025': 0.0052, 'Dec_2025': 0.0055, 'Jan_2026': 0.0055, 'Feb_2026': 0.0055
        }
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, sample_weights=None):
        ...
        try:
            print(f"    Calibrating probabilities (sigmoid, cv=5)...")
...
def main():
    ...
    month_map = {'May':'05', 'Jun':'06', 'Jul':'07', 'Aug':'08', 'Sep':'09',
    ...
    for idx, month_config in enumerate(config['months_to_process'], 1):
        ...
        month_short, year = predict_month.split('_')
        mm_yyyy = f"{month_map[month_short]}{year}"
        ...
        try:
            ...
            # Split patients by ID (prevents patient-level leakage - QA FIX)
            ...
            for i, row in train_df.iterrows():
                ...
                forecast_data.append({
                    ...
                    'Flagged': prob_rounded >= adaptive_threshold
                })
        ...
        if detected_test and os.path.exists(detected_test):
            ...
            try:
                ...
                if y_actual is not None:
                    ...
                    if matched:
                        y_pred_proba = np.array([forecast_indexed.loc[a, 'Probability'] for a in matched])
