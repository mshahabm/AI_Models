...
class FallRiskForecastingModel:
    ...
    def _create_model(self, class_weights=None):
        ...
        if self.algorithm == 'XGBoost':
            ...
            return RandomForestClassifier(
                ...
                min_samples_leaf=1,
                max_features='sqrt',
                class_weight={0: 1, 1: 150},  # Increased from 120 to 150 for higher recall
                criterion='entropy',
                bootstrap=True,
                oob_score=True,
    ...
    def train(self, X_train, y_train, sample_weights=None):
        ...
        # FIX Issue #4: Calculate optimal threshold from training data
        ...
        # FIX: For sparse activity data, use low threshold to prioritize recall
        if self.optimal_threshold > 0.004:
            original_threshold = self.optimal_threshold
            self.optimal_threshold = 0.004  # Lowered from 0.006 to 0.004 for even higher recall
            print(f"    ⚠ Threshold lowered from {original_threshold:.3f} to {self.optimal_threshold:.4f} for maximum recall")
