...
import matplotlib
matplotlib.use('Agg')  # Non-interactive backend to prevent threading errors
...
class FallRiskForecastingModel:
    ...
    def _create_model(self, class_weights=None):
        ...
        if self.algorithm == 'XGBoost':
            ...
            return RandomForestClassifier(
                ...
                min_samples_leaf=1,
                max_features='sqrt',
                class_weight={0: 1, 1: 110},  # Optimized: 110 for improved recall while maintaining FPR <50% (QA Issue #2 refined)
                criterion='entropy',
                bootstrap=True,
                oob_score=True,
            ...
            if self.use_temporal_features:
                ...
                for param in ['Steps_mean_temporal', 'Steps_median_temporal', 'Steps_divergence_temporal', 'Steps_Max_temporal']:
                    if param in df_with_steps.columns:
                        # Impute NaN with 0 to handle sparse activity data
                        X[param] = df_with_steps[param].fillna(0).values
            else:
            ...
            for i in range(len(available_months) - 1):
                ...
                if self.use_temporal_features:
                    ...
                    for param in ['Steps_mean_temporal', 'Steps_median_temporal', 'Steps_divergence_temporal', 'Steps_Max_temporal']:
                        if param in df_with_steps.columns:
                            # Impute NaN with 0 to handle sparse activity data
                            X_month[param] = df_with_steps[param].fillna(0).values
                else:
                    # Legacy mode (for comparison only - contains leakage)
                    for param in ['Steps_mean', 'Steps_median', 'Steps_divergence', 'Steps_Max']:
                        if param in df.columns:
    ...
    def _add_engineered_features(self, X):
        ...
        if steps_temporal_features:
            # Create flag: 1 if any temporal Steps feature is non-zero, 0 otherwise
            X['has_activity_data'] = (X[steps_temporal_features].abs().sum(axis=1) > 0).astype(int)
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, sample_weights=None):
        ...
        # Calibrate
        self.calibrated_model = None
        print(f"    Using raw model probabilities (calibration disabled for RandomForest)")
        
        # FIX QA Issue: Calculate optimal threshold on VALIDATION set (not training)
        # Using F2 score (emphasizes recall over precision) - critical for fall detection in healthcare
        if self.optimal_threshold is None:
            if X_val is not None and y_val is not None and len(y_val) > 0:
                print(f"    Calculating optimal threshold on VALIDATION set (held-out patients)...")
                X_val_scaled = self.scaler.transform(X_val) if self.use_scaling else X_val
                if self.use_scaling:
                    X_val_scaled = pd.DataFrame(X_val_scaled, columns=X_val.columns)
                self.optimal_threshold = self._calculate_optimal_threshold(X_val_scaled, y_val, method='f2')
            else:
                print(f"    WARNING: No validation set provided. Calculating threshold on training (not recommended)...")
                self.optimal_threshold = self._calculate_optimal_threshold(X_train_scaled, y_train, method='f2')
        
        # FIX QA Issue #2: Clinical validation - use moderate threshold for sustainable FP rate
        # Balance recall with operational capacity (<50% FPR per QA recommendations)
        # Refined: Lower floor to 0.007 (0.7%) to improve recall while maintaining FPR <50%
        if self.optimal_threshold > 0.015:
            original_threshold = self.optimal_threshold
            self.optimal_threshold = 0.015  # Moderate threshold (1.5%) for clinically sustainable FP rate
            print(f"    ⚠ Threshold lowered from {original_threshold:.3f} to {self.optimal_threshold:.3f} for clinical sustainability")
        elif self.optimal_threshold < 0.007:
            original_threshold = self.optimal_threshold
            self.optimal_threshold = 0.007  # Minimum threshold (0.7%) optimized for improved recall with FPR <50%
            print(f"    ⚠ Threshold raised from {original_threshold:.4f} to {self.optimal_threshold:.3f} to balance recall/FP")
    ...
    for idx, month_config in enumerate(config['months_to_process'], 1):
        ...
        try:
            ...
            for i, row in train_df.iterrows():
                ...
                # Include ALL accounts with Flagged column to indicate high-risk (prob >= threshold)
                forecast_data.append({
                    ...
                    'Flagged': prob >= model.optimal_threshold,
            ...
            print(f"  ✓ Forecast: {total_members} members ({flagged_members} flagged at threshold={model.optimal_threshold:.3f}), Flagged: {forecast_df[forecast_df['Flagged']]['Risk_Category'].value_counts().to_dict()}")
