...
class FallRiskForecastingModel:
    ...
    def _create_model(self, class_weights=None):
        ...
        if self.algorithm == 'XGBoost':
            scale_pos_weight = 40.0  # Ultra Aggressive: increased to 40.0 for 70% recall target (200% increase from baseline)
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, sample_weights=None):
        ...
        # FIX: For sparse activity data, use even lower threshold to maximize recall
        # Lower threshold = more predictions = higher recall (catches more falls)
        # Ultra Aggressive: lowered to 0.003 for 70% recall target (extremely sensitive)
        if self.optimal_threshold > 0.003:
            original_threshold = self.optimal_threshold
            self.optimal_threshold = 0.003
            print(f"    ⚠ Threshold lowered from {original_threshold:.4f} to {self.optimal_threshold:.3f} for maximum recall (ultra-aggressive)")
