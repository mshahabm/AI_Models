...
class FallRiskForecastingModel:
    ...
    def prepare_forecasting_features(self, df, predict_month=None, patient_ids_filter=None):
        ...
        if predict_month:
            ...
            # Convert all feature columns to numeric, handling None/object types
            for col in X.columns:
                X[col] = pd.to_numeric(X[col], errors='coerce')
            
            X.columns = [col.replace(f'_{current_month}', '') for col in X.columns]
            X['age'] = pd.to_numeric(df['age'] if 'age' in df.columns else df.get('Age', 75), errors='coerce').fillna(75)
            ...
            for i in range(len(available_months) - 1):
                ...
                # Convert all feature columns to numeric, handling None/object types
                for col in X_month.columns:
                    X_month[col] = pd.to_numeric(X_month[col], errors='coerce')
                
                X_month.columns = [col.replace(f'_{current_month}', '') for col in X_month.columns]
                X_month['age'] = pd.to_numeric(df['age'] if 'age' in df.columns else df.get('Age', 75), errors='coerce').fillna(75)
    ...
    def _calculate_steps_temporal(self, df, month_order, current_month):
        ...
        if not steps_cols:
            # No step columns exist - set temporal features to NaN (not 0!)
            # This ensures XGBoost handles missing data consistently
            df['Steps_mean_temporal'] = df['Steps_median_temporal'] = df['Steps_divergence_temporal'] = df['Steps_Max_temporal'] = np.nan
            return df
        
        # Convert to numeric and handle NaN/None values
        steps_data = df[steps_cols].apply(pd.to_numeric, errors='coerce').values.astype(np.float64)
        
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", RuntimeWarning)
            # Use numpy functions with proper handling
            # Keep NaN values as-is - let XGBoost handle them natively
            ...
            # Do NOT fill NaN with 0 - XGBoost will learn optimal direction for missing values
            # This prevents members without activity data from being scored as "no activity = high risk"
        return df
    ...
    def _calculate_optimal_threshold(self, X, y, method='recall_target', target_recall=0.63):
        ...
        print(f"    Optimal: {best_threshold:.4f} (Recall={final_recall:.4f}, Precision={final_precision:.4f}, F2={final_f2:.4f})")
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, sample_weights=None, predict_month=None):
        ...
        if X_val is not None and y_val is not None:
            ...
            optimal_threshold = self._calculate_optimal_threshold(X_val, y_val, method='recall_target', target_recall=0.63)
...
def update_training_file(old_file, test_file, new_file, target_month):
    ...
    # Sort by account_number only (health_plan column doesn't exist in DTC files)
    merged_df = merged_df.sort_values('account_number').reset_index(drop=True)
...
def find_latest_training_file():
    ...
    files = glob.glob("FallRisk_Training_*_To_*_DTC.*")
    ...
    file_dates = []
    for file in files:
        match = re.search(r'_To_(\d{2})(\d{4})_DTC', file)
        if match:
...
def extract_last_month_from_file(filename):
    ...
    match = re.search(r'_To_(\d{2})(\d{4})_DTC', filename)
    if match:
...
def check_test_file_exists(month):
    ...
    pattern = f"FallRisk_Test_{month}_DTC"
...
def process_single_month(predict_month, training_file, test_file, algorithm):
    ...
    try:
        ...
        # Vectorized forecast creation (100-1000x faster than iterrows)
        forecast_df = train_df[['account_number']].copy()
        
        # Add demographic columns only if they exist in the data
        if 'age' in train_df.columns:
            forecast_df['Age'] = train_df['age']
        elif 'Age' in train_df.columns:
            forecast_df['Age'] = train_df['Age']
        
        if 'brand' in train_df.columns:
            forecast_df['brand'] = train_df['brand']
        
        if 'member name' in train_df.columns:
            forecast_df['member name'] = train_df['member name']
        
        if 'care manager' in train_df.columns:
            forecast_df['care manager'] = train_df['care manager']
        
        # Map probabilities and data quality using vectorized operations
        forecast_df['Probability_Raw'] = forecast_df['account_number'].map(prob_dict).fillna(0.0)
        forecast_df['Data_Quality_Pct'] = forecast_df['account_number'].map(data_quality_dict).fillna(0.0) * 100
        forecast_df['Probability'] = forecast_df['Probability_Raw'].round(4)
        forecast_df['Flagged'] = forecast_df['Probability_Raw'] >= adaptive_threshold
        forecast_df['Data_Quality_Pct'] = forecast_df['Data_Quality_Pct'].round(1)
        ...
        # Save forecast in format matching input file
    ...
    if test_file and os.path.exists(test_file):
        ...
        try:
            ...
            if y_actual is not None:
                ...
                # Use vectorized index intersection (5-10x faster)
                matched = forecast_indexed.index.intersection(test_df['account_number'])
                
                if len(matched) > 0:
                    # Vectorized array creation (10-50x faster than list comprehension)
                    y_pred_proba = forecast_indexed.loc[matched, 'Probability_Raw'].values
                    y_pred = forecast_indexed.loc[matched, 'Flagged'].astype(int).values
                    
                    # Create efficient lookup for y_actual
                    test_indexed = test_df.set_index('account_number')
                    y_true = test_indexed.loc[matched, fall_col].values > 0
        ...
        try:
            ...
            if is_parquet_input and HAS_PARQUET:
                # Parquet input -> Parquet output only
                new_training_file = f"FallRisk_Training_{start_month}_To_{mm_yyyy}_DTC.parquet"
                ...
                # CSV input -> CSV output only
                new_training_file = f"FallRisk_Training_{start_month}_To_{mm_yyyy}_DTC.csv"
                update_training_file(training_file, test_file, new_training_file, predict_month)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
EXAMPLES:
  python FallRisk_DTC_Manual.py --predict_month 05_2025 --training_file FallRisk_Training_022024_To_042025_DTC
  python FallRisk_DTC_Manual.py --auto
        """
    )
        ...
        while iteration <= max_iterations:
            ...
            if test_file is None:
                ...
                if success:
                    ...
                    print(f"\nWaiting for: FallRisk_Test_{predict_month}_DTC")
